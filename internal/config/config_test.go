...
func TestParseServerConfig(t *testing.T) {
	tests := []struct {
		...
		{
			name: "custom address",
			args: []string{"-a=localhost:9090", "-i=10", "-f=/tmp/metrics.json", "-d=postgres://user:pass@localhost/db", "-k=secret", "-r=false", "--audit-file=/tmp/audit.log", "--audit-url=https://audit.example/events"},
			want: ServerConfig{
